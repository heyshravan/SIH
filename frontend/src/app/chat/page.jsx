...
function ChatContent() {
  ...
  return (
    ...
    >
      ...
      {pasteNotification && (
        <div className="fixed top-6 right-6 z-50 px-5 py-3 rounded-2xl bg-emerald-500 text-white font-black text-sm shadow-2xl flex items-center gap-2.5 animate-bounce">
      ...
      <aside
        className={`${
          sidebarOpen ? "w-68 translate-x-0" : "-translate-x-full md:translate-x-0 md:w-16"
      ...
      >
        ...
        <div className={`p-3.5 border-b ${isDark ? "border-white/10" : "border-slate-200"} flex items-center justify-between`}>
          <Link href="/" className="flex items-center gap-1.5 overflow-hidden px-1">
            {sidebarOpen && (
              <span className={`font-black text-xl tracking-tight whitespace-nowrap ${
                ...
                SatQuery <span className="text-violet-500 text-xs font-mono font-extrabold">AI</span>
              </span>
            )}
          </Link>
        ...
        <div className="p-3.5">
          <button
            ...
            className={`w-full py-3.5 px-4 rounded-xl border flex items-center gap-3 font-black text-xs tracking-wide transition-all shadow-md ${
          ...
          >
            <Plus className="w-5 h-5 text-violet-500 shrink-0 stroke-[3]" />
            {sidebarOpen && <span className="text-sm font-extrabold">New chat</span>}
          </button>
        </div>
              ...
              {["Today", "Yesterday", "Previous 30 Days"].map((groupDate) => {
                ...
                return (
                  <div key={groupDate} className="space-y-1.5">
                    <p className={`text-xs font-mono font-extrabold px-2 uppercase tracking-wider ${
                      ...
                      return (
                        <div key={h.id} className="relative group/item flex items-center">
                          <button
                            ...
                            className={`w-full text-left px-3.5 py-3 rounded-xl text-xs sm:text-sm font-bold truncate transition-all flex items-center gap-3 ${
                              isActive
                                ? isDark
                          ...
                          >
                            ...
                            <span className="truncate flex-1 font-bold">{h.title}</span>
                          </button>
        ...
        {sidebarOpen && (
          <div className="px-3.5 py-2.5 border-t dark:border-white/10 border-slate-200">
        ...
        <div className={`p-3.5 border-t ${isDark ? "border-white/10" : "border-slate-200"} space-y-2.5`}>
          {sidebarOpen && (
            <div className="space-y-1.5 text-xs sm:text-sm font-bold">
              ...
              >
                <Home className="w-4.5 h-4.5 text-cyan-500" />
                <span className="font-extrabold">Return to Home</span>
              </Link>
              <button
                onClick={() => setChangeDetectionMode(!changeDetectionMode)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-colors font-extrabold ${
                  changeDetectionMode
              ...
              >
                <GitMerge className="w-4.5 h-4.5 text-indigo-400" />
          ...
          <div className="flex items-center justify-between pt-1">
            <div className="flex items-center gap-3 truncate">
              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-violet-600 to-cyan-500 flex items-center justify-center text-white font-black text-sm shrink-0 shadow-md">
                SQ
              </div>
              {sidebarOpen && (
                <div className="truncate">
                  ...
                  <p className="text-[11px] font-mono text-cyan-500 font-bold truncate">Agentic QLoRA</p>
                </div>
              )}
            </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto p-3 sm:p-6 space-y-6 relative">
        ...
        {messages.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center max-w-4xl mx-auto text-center space-y-8 py-8 sm:py-16 px-3">
            {/* Glowing Gradient Heading */}
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight font-sans bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-indigo-950 to-violet-800 dark:from-white dark:via-slate-100 dark:to-cyan-300 leading-tight">
              What satellite analysis can I help with today?
            </h2>

            {/* Glassmorphic Segmented Mode Switcher Pill */}
            <div className={`flex items-center p-2 rounded-full border shadow-2xl backdrop-blur-md ${
              isDark
              ...
              <button
                ...
                className={`flex items-center gap-2.5 px-5 sm:px-6 py-2.5 rounded-full text-xs sm:text-sm font-black transition-all duration-300 ${
                  mode === "instant"
              ...
              {presetSamples.map((s, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`group p-5 rounded-2xl border text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl flex flex-col justify-between space-y-3 relative overflow-hidden ${
                      isDark
                  ...
                  >
                    <div className="flex items-start justify-between w-full">
                      <div className="flex items-center gap-3.5">
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-tr ${s.color} flex items-center justify-center text-white shadow-md shrink-0 group-hover:scale-110 transition-transform`}>
                          <IconComponent className="w-5 h-5" />
                        </div>
                        <span className="font-black text-base sm:text-lg tracking-tight">{s.name}</span>
                      </div>
                      <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-violet-500 group-hover:translate-x-1 transition-all shrink-0" />
                    </div>
                    <p className={`text-xs sm:text-sm leading-relaxed font-semibold text-left ${
        ...
        {/* Messages Thread (Expanded Font Sizes & Width) */}
        {messages.length > 0 && (
          <div className="flex-1 space-y-4 sm:space-y-6 max-w-5xl mx-auto w-full">
            {/* Mode Switcher Banner */}
            <div className="flex justify-center pb-2">
              <div className={`flex items-center p-1.5 rounded-full border shadow-md ${
              ...
              <div key={msg.id} className="space-y-3">
                ...
                {msg.sender === "user" && (
                  <div className="flex justify-end my-4">
                    <div className="max-w-2xl bg-[#0055ff] dark:bg-[#1d4ed8] text-white px-6 py-3.5 rounded-3xl text-base sm:text-lg font-bold shadow-lg space-y-3 leading-relaxed">
                      <div className="flex items-center gap-3">
                        {msg.image && (
                          <div className="rounded-2xl overflow-hidden max-w-xs border border-white/20 mb-2 relative bg-slate-900">
                            ...
                            />
                            <span className="block text-xs font-mono text-center bg-black/70 text-white py-1 font-bold">T1 (Before)</span>
                          </div>
                        )}
                        {msg.image2 && (
                          <div className="rounded-2xl overflow-hidden max-w-xs border border-white/20 mb-2 relative bg-slate-900">
                            <img
                              ...
                              className="w-full h-40 object-cover"
                            />
                            <span className="block text-xs font-mono text-center bg-black/70 text-white py-1 font-bold">T2 (After)</span>
                          </div>
                        )}
                      </div>
                      <p>{msg.text}</p>
                ...
                {/* Assistant Response Layout (Enlarged Text) */}
                {msg.sender === "assistant" && (
                  <div className="flex flex-col items-start my-5 space-y-3.5 max-w-4xl">
                    {msg.isError ? (
                      /* Error Bubble with Try Again Button */
                      <div className="p-4 sm:p-5 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-800 dark:text-rose-200 text-base font-bold flex items-center justify-between gap-4 w-full">
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-black text-xs sm:text-sm flex items-center gap-2 shadow-md shrink-0 transition-all hover:scale-105 active:scale-95"
                        >
                      ...
                      <div className="space-y-4 w-full">
                        {/* Model / Task Tag */}
                        <div className="flex items-center gap-2.5 text-xs sm:text-sm font-mono font-black">
                          <Badge variant="indigo" className="text-xs px-3 py-1 font-mono">{msg.model}</Badge>
                          <span className="text-cyan-600 dark:text-cyan-400 font-extrabold">{msg.task}</span>
                        </div>

                        {/* Reasoning Log */}
                        {msg.thinking && (
                          <div className={`p-4 sm:p-5 rounded-2xl border text-xs sm:text-sm font-mono space-y-2 ${
                            isDark ? "bg-black/50 border-white/15 text-slate-200" : "bg-slate-100 border-slate-300 text-slate-800"
                          }`}>
                            <p className="text-violet-500 dark:text-violet-400 font-black text-sm flex items-center gap-2">
                              <Brain className="w-4.5 h-4.5" /> Agent Controller Reasoning
                            </p>
                            <pre className="whitespace-pre-wrap leading-relaxed font-mono font-medium">{msg.thinking}</pre>
                          </div>
                        )}

                        {/* Plain Clean Assistant Answer Text (Enlarged to text-base / text-lg) */}
                        <div className={`text-base sm:text-lg md:text-xl leading-relaxed font-semibold ${
                        ...
                        {(msg.resultImage || msg.resultImage2) && (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-3xl mt-3">
                            {msg.resultImage && (
                              <div className="rounded-2xl overflow-hidden border-2 bg-black relative aspect-video shadow-xl border-slate-300 dark:border-white/20">
                                <img
                                  src={msg.resultImage}
                                      ...
                                      return (
                                        <g key={bIdx}>
                                          ...
                                          <foreignObject
                                            ...
                                            height="28"
                                          >
                                            <div className="bg-red-950/90 text-red-100 text-xs font-mono px-2.5 py-1 rounded-md border border-red-500/80 inline-flex items-center gap-1.5 shadow-xl font-black whitespace-nowrap">
                                              <span>🎯 {box.label || 'Target'}</span>
                                              {box.confidence && (
                                                <span className="text-amber-300 font-extrabold">{box.confidence}%</span>
                                              )}
                                            </div>
                                          </foreignObject>
                                        </g>
                        ...
                        {/* Action Icons Row */}
                        <div className="flex items-center gap-4 pt-1.5 text-slate-400 dark:text-slate-500">
            ...
            {isGenerating && (
              <div className="flex items-center justify-between p-4 rounded-2xl dark:bg-white/5 bg-slate-100 border dark:border-white/10 border-slate-200 max-w-lg my-4">
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 rounded-full bg-violet-500 animate-ping shrink-0" />
                  <span className="text-sm font-mono font-extrabold text-slate-700 dark:text-slate-200">
                    Processing satellite query...
                  </span>
                </div>
                <button
                  onClick={handleStopQuery}
                  className="px-3.5 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-black text-xs flex items-center gap-1.5 shadow-md transition-all hover:scale-105 active:scale-95"
                >
        ...
        {/* Floating Input Container (Enlarged) */}
        <div className="max-w-4xl mx-auto w-full pt-2">
          ...
          {(attachedImage || attachedImage2) && (
            <div className="flex items-center gap-2 mb-2 overflow-x-auto">
              {attachedImage && (
                <div className={`flex items-center gap-2.5 p-2.5 rounded-xl text-xs font-black shadow-lg border ${
                  isDark ? "bg-[#141628] border-violet-500/30 text-white" : "bg-white border-slate-300 text-slate-900"
                }`}>
                  <img src={attachedImage.url} alt="T1 Attached" className="w-9 h-9 rounded-lg object-cover" />
                  <span className="font-mono text-xs text-cyan-500">T1: {attachedImage.name}</span>
              ...
              {attachedImage2 && (
                <div className={`flex items-center gap-2.5 p-2.5 rounded-xl text-xs font-black shadow-lg border ${
                  ...
                  <img src={attachedImage2.url} alt="T2 Attached" className="w-9 h-9 rounded-lg object-cover" />
                  <span className="font-mono text-xs text-indigo-400">T2: {attachedImage2.name}</span>
          ...
          <div className={`rounded-3xl p-4 sm:p-5 shadow-2xl border-2 transition-colors space-y-3.5 ${
            isDark
            ...
            <textarea
              ...
              className="w-full bg-transparent text-sm sm:text-base font-bold placeholder-slate-400 focus:outline-none resize-none px-2 dark:text-white text-slate-900"
            />

            {/* Bottom Input Action Controls */}
            <div className="flex items-center justify-between pt-2.5 border-t dark:border-white/10 border-slate-200 gap-2">
              <div className="flex items-center gap-2 overflow-x-auto">
                <button
                  onClick={() => setAgentThink(!agentThink)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black transition-all whitespace-nowrap ${
                    agentThink
              ...
              <div className="flex items-center gap-2 shrink-0">
                {changeDetectionMode && attachedImage && (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-full bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-300 border border-indigo-500/40 font-mono text-xs font-black flex items-center gap-1"
                  >
                ...
                <button
                  ...
                  className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border dark:bg-white/10 dark:hover:bg-white/20 dark:border-white/20 dark:text-white bg-slate-100 hover:bg-slate-200 border-slate-300 text-slate-800 flex items-center justify-center transition-colors shadow-sm"
                >
                  <Plus className="w-5 h-5 text-cyan-500 dark:text-cyan-400 stroke-[3]" />
                </button>

                <button
                  onClick={() => fileInputRef.current?.click()}
                  title="Attach File or Paste from Clipboard"
                  className="w-9 h-9 sm:w-10 sm:h-10 rounded-full dark:bg-white/10 dark:hover:bg-white/20 dark:text-white bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition-colors"
                >
                ...
                {isGenerating ? (
                  <button
                    ...
                    className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-rose-600 hover:bg-rose-500 text-white flex items-center justify-center shadow-lg shadow-rose-500/30 transition-all hover:scale-105 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white flex items-center justify-center shadow-lg shadow-indigo-500/30 transition-all disabled:opacity-40"
                  >
